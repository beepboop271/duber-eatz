...
class DuberEatzGraphzExperimentCopy {
  ...
  public static final int[] NO_PATH = {
    Integer.MAX_VALUE,
    Integer.MIN_VALUE
  };
  public static int numPOIs = 0;
  ...
  public static void main(String[] args) throws InterruptedException, IOException {
    ...
    int[] result = findPath(new byte[numPOIs],
  ...
  public static int[] findPath(byte[] visited,
    ...
    if(visited[id] == 1 || visited[id] == 2) {
      ...
      if(currentNode.cell >= '0' && currentNode.cell <= '9' && visited[id] != 2) {
        --destsLeft;
        ...
        newVisited[0] = 0;
        newVisited[id] = 2;
      } else if(currentNode.cell == 'M' && visited[id] != 3) {
        steps = realSteps/2;
        // newVisited = getMapCopy(visited, true);
        newVisited = visited.clone();
        newVisited[0] = 0;
      ...
      for(int i = 0; i < currentNode.numEdges; ++i) {
        int nextId = currentNode.getNeighbourNode(i).id;
        if(!(visited[nextId] == 1 || visited[nextId] == 2)) {
  ...
  public static void exploreGraph(Node currentNode, byte[][] visited, boolean shouldPrint) {
    ...
    if(visited[row][col] == 1) {
      ...
      ++tmpNumNodes;
      if(shouldPrint) {
        System.out.printf("N%d (%d, %d) cell:'%c' edges:%d:\n",
                          currentNode.id,
                          col, row,
                          currentNode.cell,
