 ...
 * @version 10
 * @author Kevin Qiao
 * Code for the Duber Eatz assignment lvl 4++
 */

class DuberEatz4PlusPlus {
  ...
  public static void main(String[] args) throws IOException {
    ...
    // store the best tips that were obtained for each state
    // so that if a destination is reached with the same state
    // but worse tips, the program can stop exploring
   ...
   * the best path.
   * @param bestTips      Int array storing the best tips for each
   *                      state visited.
   ...
   */
  public static int[] findPath(int[] bestTips,
                               int[] path,
    ...
    if ((currentNode.cell >= '0')
          && (currentNode.cell <= '9')
          && (used > -1)
          // array must be < 2^30, so used must be within 0 and 1<<30
          && (used < 0x40_00_00_00)) {
      // if at a destination, and the same deliveries and microwaves
      // have been done with better tip, no point continuing this
      // path since it will be explored in another recursive call
    ...
    for (int i = 0; i < currentNode.numEdges; ++i) {
      ...
      if (((visited&nextIdMask) == 0)     // dont explore visited areas
        ...
        result = findPath(bestTips,
                          ...
                          useMicrowaves,
                          isDense);
        // record the result if the tips are more
